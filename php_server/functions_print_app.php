...
function get_sort_list() {
  ...
  if ($cached_list == FALSE) {
      ...
      $results = $db->query('select * from products order by Nosaukums ASC;');
      $data = array ();
      while ($row = $results->fetchArray()) {
            $list_item = [
               "id" => $row['id'],
                "Nosaukums" => $row['Nosaukums'],
                "LatinNosaukums" => $row['LatinNosaukums'],
                "Potcelms" => $row['Potcelms'],
                "Zona" => $row['Zona'],
                "Kods1" => $row['Kods1'],
                "Suga" => $row['Suga'],
                "PasesKrasa" => $row['PasesKrasa'],
...
function get_count_suggestion_array () {
